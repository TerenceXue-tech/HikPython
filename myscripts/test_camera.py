...
import cv2
from ctypes import (
    POINTER, CFUNCTYPE, c_ubyte, c_void_p,
    cast, byref, memmove, memset
)
    ...
    if active_way == "getImagebuffer":
        ...
        while True:
            ...
            if ret != 0 or stOutFrame.pBufAddr is None:
                ...
                # 即使失败，也建议不要 FreeImageBuffer（但很多示例仍然会调用；按你原逻辑保留也行）
                continue

            info = stOutFrame.stFrameInfo
            frame_len = int(getattr(info, "nFrameLen", 0))

            # 兜底：如果结构体没有 nFrameLen（极少数旧版本），再退回到像素类型推算
            if frame_len <= 0:
                # 保持你原来的推算方式作为退路
                if info.enPixelType in (17301505, 17301514):
                    frame_len = int(info.nWidth * info.nHeight)
                elif info.enPixelType == 35127316:
                    frame_len = int(info.nWidth * info.nHeight * 3)
                elif info.enPixelType == 34603039:
                    frame_len = int(info.nWidth * info.nHeight * 2)
                else:
                    # 仍然不知道长度，就把关键字段打印出来，方便你定位像素类型
                    print("unknown PixelType[%d], Width[%d], Height[%d], ret[0x%x]" %
                          (int(info.enPixelType), info.nWidth, info.nHeight, ret))
                    cam.MV_CC_FreeImageBuffer(stOutFrame)
                    continue

            print("get one frame: Width[%d], Height[%d], nFrameNum[%d], PixelType[%d], FrameLen[%d]" %
                  (info.nWidth, info.nHeight, info.nFrameNum, int(info.enPixelType), frame_len))

            pData = (c_ubyte * frame_len)()
            memmove(byref(pData), stOutFrame.pBufAddr, frame_len)

            data = np.frombuffer(pData, count=frame_len, dtype=np.uint8)
            image_control(data=data, stFrameInfo=info)
        ...
        while True:
            ...
            if ret == 0:
                print("get one frame: Width[%d], Height[%d], nFrameNum[%d], PixelType[%d]" %
                      (stFrameInfo.nWidth, stFrameInfo.nHeight, stFrameInfo.nFrameNum, int(stFrameInfo.enPixelType)))
...
def image_callback(pData, pFrameInfo, pUser):
    ...
    if img_buff is None and stFrameInfo.enPixelType == 17301505:
        ...
        data = np.frombuffer(img_buff,
                             count=int(stFrameInfo.nWidth * stFrameInfo.nHeight),
                             dtype=np.uint8)
        ...
        memmove(byref(img_buff), pData, stFrameInfo.nWidth * stFrameInfo.nHeight * 2)
        data = np.frombuffer(img_buff,
                             count=int(stFrameInfo.nWidth * stFrameInfo.nHeight * 2),
                             dtype=np.uint8)
        image_control(data=data, stFrameInfo=stFrameInfo)
        del img_buff
...
# Ubuntu 下必须用 CFUNCTYPE
EventInfoCallBack = CFUNCTYPE(None, stEventInfo, c_void_p)
...
def main():
    ...
    if int(stdcall) == 0:
        ...
        # 当使用 回调取流时，需要在此处添加
        print("press Enter to stop grabbing.")
        sys.stdin.readline()   # 阻塞，等待用户按回车
